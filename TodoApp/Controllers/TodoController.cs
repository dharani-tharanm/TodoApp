...
namespace TodoApp.Controllers
{
    public class TodoController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            try
            {
                tasks = await _httpClient.GetFromJsonAsync<List<Todo>>("https://localhost:44362/api/Tasks");
                if (tasks == null)
                {
                    tasks = new List<Todo>();
        ...
        public async Task<IActionResult> Create(Todo todo)
        {
            ...
            try
            {
                var response = await _httpClient.PostAsJsonAsync("https://localhost:44362/api/Tasks", todo);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                var response = await _httpClient.DeleteAsync($"https://localhost:44362/api/Tasks/{id}");
                response.EnsureSuccessStatusCode();
            }
            catch (HttpRequestException)
            {
